...
describe("activity helpers", () => {
  describe("buildActivityEvents", () => {
    it("builds events from cargas and status", () => {
      ...
      expect(
        events.some((event) => event.description.includes("Viagem 1")),
      ).toBe(true);
      expect(
    ...
    it("keeps unknown dates as 'Sem data' and out of today's count", () => {
      const events = buildActivityEvents(
        [{ id_viagem: "1", created_at: undefined, prev_coleta: undefined }],
        null,
      );

      expect(events[0].time).toBe("Sem data");
      expect(events[0].timestamp).toBeNull();
      expect(countTodayEvents(events, new Date("2026-02-17T12:00:00Z"))).toBe(
        0,
      );
    });
  });

  describe("countActivityAlerts", () => {
